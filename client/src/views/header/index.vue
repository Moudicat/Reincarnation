<template>
  <header>
    ...
    <div
      ...
      :class="{ 'article-mode': articleMode }"
      ...
      <Sticky v-if="!articleMode">
        <div class="nav-wrapper">
          <div class="nav-container">
            ...
            <div
              class="profile-mini-wrapper"
              :class="{ mini: miniAvatarState }"
            >
              <div class="profile-mini">
                <img :src="avatarUrl" alt="mini-avatar" />
                <div class="info">
                  <h3>结束与起始之界</h3>
            ...
            <transition-group name="component-fade" tag="nav">
              <router-link
                :to="nav.link"
                class="nav-link"
                v-for="(nav, index) in navMap"
                :key="nav.name + index"
                v-if="navAnimationIndex >= index"
              >
                <span>{{ nav.name }}</span>
              </router-link>
            </transition-group>
...
export default {
  ...
  methods: {
    weatherChange(name) {
      ...
      this.$refs.backgroundWrapper.removeChild(
